...
def plot_data(data, sex_index):
    ...
    male_data = [row for row in data if row[sex_index] == MALE_ENUM]
    female_data = [row for row in data if row[sex_index] == FEMALE_ENUM]
    ...
    male_prevalence = list(map(prevalence_fn, instantaneous))
    male_incidence = list(map(incidence_fn, list(zip(cumulative, instantaneous))))
    male_mortality = list(map(mortality_fn, list(zip(cumulative, instantaneous))))
    males_on_art = list(map(on_art_fn, instantaneous))

    (cumulative, instantaneous) = aggregate_and_extract(female_data)

    female_prevalence = list(map(prevalence_fn, instantaneous))
    female_incidence = list(map(incidence_fn, list(zip(cumulative, instantaneous))))
    female_mortality = list(map(mortality_fn, list(zip(cumulative, instantaneous))))
    females_on_art = list(map(on_art_fn, instantaneous))

    timeline = [row[0] for row in cumulative]

    figure = pyplot.figure('HIV Summary', figsize=(12,4))
...
def do_plot(male, female, timeline, plot_definition):
    ...
    pyplot.plot(timeline, male, 'b', label='Male', linewidth=1.0)
    pyplot.plot(timeline, female, 'r', label='Female', linewidth=1.0)
